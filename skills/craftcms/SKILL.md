---
name: craftcms
description: "Craft CMS 5 plugin and module development — extending Craft with PHP. Covers elements, element queries, services, models, records, project config, controllers, CP templates, migrations, queue jobs, console commands, field types, native fields, events, behaviors, Twig extensions, utilities, widgets, filesystems, permissions, debugging, testing, GraphQL, and Craft configuration. Triggers on: beforePrepare(), afterSave(), defineSources(), defineTableAttributes(), attributeHtml(), MemoizableArray, BaseNativeField, EVENT_REGISTER_*/DEFINE_*/BEFORE_*/AFTER_*, CraftVariable, registerTwigExtension, custom element type, custom field type (normalizeValue, serializeValue, inputHtml), webhook, API endpoint, queue job, batch processing, CP section, control panel, element action, element exporter, element condition, dashboard widget, utility page, registerUserPermissions, requirePermission, GraphQL custom types/mutations, schema building, defineRules, canView/canSave/canDelete authorization, session invalidation, elevated session, BaseCondition, system messages, Mailer, atomic deploy, craft up, project-config/apply, drafts, revisions, GeneralConfig, $allowAnonymous, set relation via DB, relations table, field value empty after DB insert, hero/teaser image not showing, elements_sites.content, layout element UID, multi-instance field value, building a Formie form in a content migration, programmatic Formie form/field/notification creation, Formie File Upload volume + file rename via Submission EVENT_AFTER_SAVE, SEOmatic SEO field in PHP, Feed Me import job, Blitz purge from a service, Imager-X transforms in a queue job, element edit sidebar panel, EVENT_DEFINE_SIDEBAR_HTML markup, EVENT_DEFINE_ADDITIONAL_BUTTONS, split button toolbar, .meta vs .meta read-only, metaFieldsHtml override, Cp::metadataHtml, disclosure menu menu-item-label, EVENT_REGISTER_ELEMENT_TYPES scope, element/field type registration context, trashed elements never deleted, gc/run not purging custom elements, getAllElementTypes empty in console, hardDeleteElements skips type, operational settings in project config vs DB, thresholds/notification routing/workflow mappings storage, project config source of truth, YAML DB divergence, per-environment tuning settings, settings screen permission-gated not admin-gated, requireAdmin vs requirePermission, manageSettings, settings permission, allowAdminChanges writability, read-only settings, getCpNavItem dead nav item, permission handle constant, CP tabs, namespace id tab trap, lightswitch not checkbox, lightswitch vs checkbox hidden input, single boolean setting control, button modifier classes .submit/.secondary/.caution/.dashed, copytextField API key/token/webhook, selectizeField searchable select, editable table field forms.editableTableField, inner sidebar nav _includes/nav.twig, CP design tokens --radius/--ui-control-height/--fg-subtle/--padding, semantic status tokens --bg-*/--fg-*, spinner .spinner-absolute, modal-shade not .shade, no Craft.confirm() method, Craft.CpModal cpmodal-body, status-badge draft indicator not empty-value badge, off vs suspended vs expired status, relation field viewMode/previewMode/maxRelations/maintainHierarchy, large is legacy alias for thumbs, field-layout UI elements Tip/Heading/LineBreak/HorizontalRule, .fld-ui-element wrapper, getChipLabelHtml/getCardBodyHtml, suggestEnvVars, parseEnv, info popover, span class=info, note warning callout, field instructions, VueAdminTable, unbounded admin table, CRAFT_ env override precedence, anonymous action failure redirect, invisible flash, enumeration-safe flow, session-carried form state, same-site returnUrl validation, project-config write in tests StaleResourceException, getUserIP X-Forwarded-For request IP fixture, passwordResetRequired console user, example-templates console command, fluent BaseTag render builder, storing IP addresses, anonymizeIp, docs/privacy.md, personal data retention. For any plugin-specific PHP, migration, or deployment work, also load the craft-plugins skill and read its references/<plugin>.md. Always use when writing, editing, or reviewing Craft CMS plugin or module PHP code, even when no specific API names are mentioned. Do NOT trigger for front-end Twig (craft-site), content modeling (craft-content-modeling), or headless GraphQL consumption from Next.js/Nuxt/Astro."
---
