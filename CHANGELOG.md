...
- **craft-feature-builder agent** — Tests woven into build gates instead of batched at the end. Gate 3 (service) now requires service tests before moving to controller. Gate 4 (controller) requires HTTP tests. Gate 7 is now a full-suite regression run, not the first time tests are written.
- **craft-planner agent** — New rule: "Tests are part of each layer, not a trailing step." Never plan a standalone "write all tests" step — tests belong in the same step as the code they verify. Best verification gate is a passing test, not a curl command.
- **craft-project-setup SKILL.md** — New Step 3b: generates `.claude/settings.local.json` with pre-approved permissions for DDEV commands, git operations, and `gh`. Connected projects question asks if related folders need permissions (plugin dev site, headless frontend, shared modules). Generated CLAUDE.md now includes a Permissions section explaining the local settings file.
