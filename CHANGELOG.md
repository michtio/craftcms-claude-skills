...
- **craft-code-reviewer agent** — Added Bash (read-only: `git diff`, `git log`, `git show`, `git blame`). Removed `craft-garnish` from default skills — loaded conditionally only when diff contains JS files. Token efficiency note.
