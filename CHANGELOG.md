...
- **`skills/craft-pest/references/isolation.md`** — the invisible half of test isolation: forcing the database name is **not sufficient** on a shared MySQL server. Craft's project-config lock is `App::dbMutexConfig()` → `MysqlMutex` with `keyPrefix = getEnvId()` (`App.php:1141`), and MySQL `GET_LOCK` names are **server-scoped, not database-scoped** — so every unpinned suite shares the env id `CraftCMS--production` (craft-pest's bootstrap `define`s it) and deadlocks on the same lock names even from separate, exclusively-owned databases (`BusyResourceException`, `1213 Deadlock`, `1412 Table definition has changed`). Fix: pin `CRAFT_ENVIRONMENT` uniquely per plugin in `phpunit.xml.dist` — never `CRAFT_APP_ID`, which only exists in the craftcms/craft starter's `config/app.php` and is inert in a plugin repo. Includes the held-lock controlled experiment for verifying which pin takes, and why pinning is safe (`getEnvId()` has exactly three process-local readers).
- **`skills/craft-plugin-release/SKILL.md`** — three new areas. Shared-library releases: dependency-first ordering (release the library, verify p2, then the consumer), the additive-minor hazard (a caret constraint admits new library code before the consumer's migration exists; a best-effort `catch (Throwable)` around bookkeeping writes turns the schema mismatch into invisible data loss on every request), release notes must say when a consumer migration is required, and library-shipped Yii modules having **no CLI migration track** (`--track=module:` throws `Invalid migration track`; consumers must call the module's `getMigrator()->up()` from their own dated migration and bump `schemaVersion`, verified at `MigrateController.php:467`). Release-artifact verification: `gh release create --notes-file <empty>` creates a blank body and exits 0 (verify with `gh release view <tag> --json body -q '.body' | wc -c`), single-regex changelog extraction instead of chained `sed`, and Craft's Store-triggered `create-release.yml` creating the release itself (`422 already_exists` if hand-created first; `allowUpdates: true` makes either order safe; the dispatch only fires post-listing).
- **`skills/craft-plugin-release/references/history-rewrites.md`** — new reference: `git filter-repo` purges on published plugin repos. Blob-level sweeps across all history (path-scoped purges have now missed content twice), the `already_ran` interactive prompt becoming an `EOFError` under a non-tty, the post-rewrite `_compute_metadata` `AssertionError` firing **after** the rewrite succeeded (check refs before concluding no-op), `--refs` + `--partial` scoping (tags are refs too), and tip-tree-hash comparison as the shipped-content-unchanged proof.
- **`skills/craftcms/references/identity-protocols.md`** — new reference: protocol domain knowledge for SSO/provisioning plugin features, all vendor-verified. Back-channel vs front-channel logout (back-channel needs a stored `sid` → local-session mapping or it's a no-op), never deriving the signing-alg allowlist from `id_token_signing_alg_values_supported` (a live Keycloak advertises `HS256` there — deriving reopens the HMAC downgrade), a single global issuer not being a tenancy boundary (Google's issuer serves personal accounts; only `hd` bounds an org, so generic-OIDC + JIT provisioning is open registration), SCIM `active: false` being a routine directory event rather than a termination (map to suspend, never delete), RFC 7644 §3.5.2 requiring in-order atomic PATCH application (type-bucketing breaks the RFC's own remove-then-add example), and documenting vendor-vs-RFC deviations at the accommodation site.
- **`skills/craftcms/references/migrations.md`** — modules and library-shipped modules have no CLI migration track (with the consumer-side dated-migration pattern), and a new rule: never call `ProjectConfig::flush()` inside a migration — `saveModifiedConfigData()` releases the shared project-config mutex the surrounding process still believes it holds (`ProjectConfig.php:873`), desyncing lock state for the rest of the process, and automatic YAML writing rewrites the developer's VCS-tracked `config/project/` files. Coerce bad stored values at read time instead.
- **`skills/craftcms/references/field-types-custom.md`** — what `settingsAttributes()` actually includes: every public non-static property whose **declaring class is not abstract**, so concrete-subclass properties need no override, abstract bases are the only legitimate reason for an explicit list, and redeclaring an inherited property to change its default duplicates the name in the merged array (`array_unique()` is the conservative fix; removing the redeclaration would silently change the default).
- **`skills/craftcms/references/elements.md`** — `getStatus()` is a display priority, not a state model: `User::getStatus()` reports `SUSPENDED` before the `active` flag is consulted, so a suspended-and-deactivated account (reachable by suspending an already-deactivated user) hides `INACTIVE`, and a restore-access flow branching on `getStatus()` unsuspends, stops, and leaves the account locked out. Read the underlying flags when multiple states matter.
- **`skills/craft-pest/references/patterns.md`** — two named practices: **a test can encode the bug** (replacing a test that pinned the defect is the fix being pinned, but must be called out explicitly in review or it reads as a regression signal), and **prove a fix or guard is not vacuous by breaking it** (revert the fix and watch the test fail; swap the calls and watch the ordering test fail; force the guard's bad condition and check the exit status — then report that the step was done).
- **`skills/craft-pest/references/ci.md`** — sweep **all** workflow files for `composer test` / `vendor/bin/pest` before asserting anything about a suite's CI status; a second, forgotten Pest job can hide in `code-analysis.yaml` beside the obvious one in `tests.yaml`.
- **`skills/craft-twig-guidelines/SKILL.md`** — pitfall: Twig has no `instanceof`, so controller-side `match (true)` class scoping silently drifts from the template's approximation; branch on a capability (`hasProperty`/`hasMethod`) or pass an explicit discriminator.
- **`skills/craftcms/references/config-app.md`** — the mail transport *type* is not env-parseable: `App::mailerConfig()` passes `$settings->transportType` (the raw class name from project config) into `createTransportAdapter()` with no `parseEnv()` (`App.php:1116`), while the settings *inside* an adapter are env-parseable (`Smtp.php:151`) — so every neighbouring Settings → Email field accepts `$VAR` and the transport dropdown looks like it should too. Project config syncing the CP choice across environments closes the other escape. The existing `app.php` conditional-override section now states this as the *reason* it is the only per-environment mechanism, plus an opt-in-flag refinement (`devMode && App::env('FORCE_MAILPIT_IN_DEV')`) so a shared `app.php` doesn't force-hijack every developer's outbound mail. New pitfall line cross-references the section.

### Fixed

- **`skills/craft-pest/references/isolation.md`** — the fail-closed database guard added in 1.13.0 used `exit(1)`, which **fails open under Pest**: Pest's shutdown handling decides the process status, so the refusal printed and the shell got `0` — green CI on the exact run the guard exists for. The guard now throws a `RuntimeException` (PHPUnit reports "Error in bootstrap script" and exits non-zero), with the break-it-and-check-`$?` proof step.

### Changed

- **`skills/craft-pest/SKILL.md`** — isolation checklist gained the `CRAFT_ENVIRONMENT` pin and throw-not-exit guard rows; trigger description now carries `CRAFT_ENVIRONMENT`, `BusyResourceException`, `GET_LOCK`, and 'two suites deadlock'.
- **`skills/craft-plugin-release/SKILL.md`** — release verification checklist now includes dependency-first ordering and the non-empty-body check; trigger description covers shared-library ordering, `Invalid migration track`, blank bodies, `422 already_exists`, and filter-repo purges.
- **`skills/craftcms/SKILL.md`** — task-routing lines for the module track, `ProjectConfig::flush()`, `getStatus()` co-existing states, `settingsAttributes()`, and the identity-protocols reference.

Deliberately left out: CraftPulse-specific mechanics (session-fixation specifics of any one plugin's SSO implementation, estate branch names) stay in the private `craftpulse-*` skills per the standing constraint; the identity material was kept to vendor-verifiable protocol facts.
...
- **`skills/craft-plugin-release/`** — new skill (the 13th): plugin release mechanics. Git tags, Packagist, and GitHub release objects drift independently while every observable signal reports success. Covers the `composer.json` `version` key (bump in the changelog-dating commit, verify via `git show <tag>:composer.json`, and the honest bump-or-omit trade-off — omitting kills the drift risk but a version-less path repo publishes `dev-<branch>`, which fails every `version_compare()` peer check), Packagist verification at `repo.packagist.org/p2/<name>.json` (never the staler `packagist.org/packages/` endpoint), the silent "Skipped tag … does not match version" failure behind a `202 Accepted` webhook, the tag recreation risk model (unserved tags safe to recreate; served tags never), `gh api PATCH` with only `body` wiping `tag_name` to `untagged-<hash>` (use `gh release edit`), `releases/latest` 404ing when everything is a prerelease, drafts minting their tag from `target_commitish`'s tip at publish time, and two-way `origin...origin` comparison before branch promotion. Plus `references/path-repositories.md`: canonical-repository semantics (Packagist versions dropped from the pool — hard failure, never a fallback), no `exclude` on wildcard entries, `extra.branch-alias` on the major line for every checkoutable branch, and duplicate package names across directories masking unsatisfiable constraints.
- **`skills/craftcms/references/cp.md`** — Settings Pages: the config-file override pattern. A field overridden by `config/<handle>.php` must stay **editable and warn** (Blitz's `configWarning`/`configFieldWarning` macro pair + one `warning:` clause per field, verified at `SettingsController.php:113`, `_macros.twig:10/:14`, `_settings.twig:90` in Blitz 5.12.9) — Craft does not surface the override automatically, and a disabled field is the wrong fix. Also: the Read-only notice section now uses core's `readOnlyNotice()` / `contentNotice` helpers instead of a hand-rolled blockquote, and `elementSelectField` documents that a cleared selection posts `''` (the macro's leading `hiddenInput`) while API clients can post an empty array — with the `reset($x) ?: null` normalization guard.
- **`skills/craftcms/references/architecture.md`** — Front-End Output From Plugins grew three pieces: render-builder attribute rules (merge `class` lists, never `array_merge`-replace; `resetClass: true` escape; guard the `data-` hooks your own JS reads; a real option allowlist instead of `method_exists()`, which accepts `render` as an "option" and dies with `ArgumentCountError`); plugin-registered CSS loading after the site's stylesheet (`EventTagAdder` inserts `head()` right before `</head>`, so site rules lose on source order at equal specificity — escape via `@layer` for cosmetic rules only, behaviour rules stay unlayered for non-supporting browsers, plus a `renderCss: false` suppression switch with Formie's three-tier design as the reference); and the JS-to-markup contract as public API (PHP resolves hooks into `data-` attributes, JS keeps literals as `||` fallbacks, docs state "what your template must provide"). Also: site deletion is a soft delete (`SoftDeleteTrait` on the Site record; FK CASCADE fires only when `Gc` hard-deletes, `Gc.php:171-173`) — plugins wanting immediate cleanup listen to `EVENT_AFTER_DELETE_SITE`.
- **`skills/craftcms/references/quality.md`** — Generated Artifacts Must Not Read Runtime Registries: generators emitting committed/published artifacts (docs, schemas, types, fixtures) must not read edition-gated registries (generator-only accessor, runtime validation untouched) or registries that accumulate install state (`getBundled()` captured before `EVENT_REGISTER_*` fires, so user-authored elements and third-party registrations can't leak into published output), and should state the resolved vendor version they were generated against.
- **`skills/craftcms/references/caching.md`** — the Yii cache `get()` `false`-vs-miss collision: string sentinels or `getOrSet()`/`exists()` for bool-domain values, with wrong/right examples.
- **`skills/craftcms/references/migrations.md`** — schema-change discipline now covers both drift directions: fresh installs run `Install.php` and mark every dated migration applied **without running it**, so a dated-only change silently never happens on new installs.
- **`skills/craftcms/references/debugging.md`** — syslog-over-TLS framing: RFC 5425 octet counting (`MSG-LEN SP SYSLOG-MSG`), and why newline framing (RFC 6587) appears to work against rsyslog's `imtcp` but fails strict receivers.
- **`skills/craft-pest/references/isolation.md`** — fail-closed database guard in `tests/bootstrap.php` (assert the *resolved* `App::env('CRAFT_DB_DATABASE')` and `exit(1)` on mismatch — one real suite had been writing to a live install and running `migrate/up` through the tool under test), and the `--filter`-subset `cookieValidationKey` harness artefact with its scoped `enableCookieValidation = false` workaround.
- **`skills/craft-pest/references/patterns.md`** — Pin behaviour before refactoring (76 pin tests on a 1,900-line controller caught a two-column-spanning search haystack, `filters[x] = ''` meaning "no filter", and null branches reachable only via soft-deleted users with `SET NULL` FKs), and the `uses()` first-match-wins registration order with the most-specific-first / no-catch-all pattern for `TestCaseAlreadyInUse`.
- **`skills/craft-pest/references/craft-state.md`** — CP-surface controller tests under a console bootstrap: the three orthogonal pins (User `idParam`, Request CP flag + host info, `assetManager.basePath`).
- **`skills/craft-php-guidelines/SKILL.md`** — console help specifics: `parseDocCommentSummary()` returns docblock line 2 *physically* (wrapped sentences truncate mid-phrase), scaffolded `* Class FooController` prints verbatim as the `craft help` summary, and `getHelpSummary()`/`getHelp()` overrides turn the docblock into dead code that disagrees with what prints. Also extends the no-dash copy rule to console command output.
- **`skills/craft-php-guidelines/references/tooling.md`** — `suggest` is prose, not a dependency (no resolution edge), and a pointer to the new skill's path-repository semantics.

### Changed

- **`skills/craftcms/SKILL.md`**, **`skills/craft-pest/SKILL.md`**, **`skills/craft-site/SKILL.md`** — routing rows and trigger descriptions for all of the above.
- **`skills/craft-site/references/third-party-integration.md`** — the integrator-side view: Plugin CSS Loads After Yours (check for a suppression switch first; unlayered beats layered; otherwise win on specificity) and Hand-Written Templates for Plugin Widgets (reproduce the plugin JS's DOM hooks or the widget renders fine and never enhances).
- **`skills/craft-site/references/example-templates.md`** — progressive-enhancement section notes the markup contract consequence for integrator-rewritten bundle pages.
- **`.claude-plugin/plugin.json`**, **`.claude-plugin/marketplace.json`**, **`README.md`**, **`docs/skills-overview.md`** — register the new skill; counts now 13 skills / 111 reference files.
...
The through-line is that **a test process is not a sequence of requests.** Craft invalidates a lot of state by ending the request; a long-lived suite that creates sites, swaps components, or re-runs an install migration carries stale memos no production path would ever see. Two of the seven turned out to be Craft-core facts rather than harness quirks, so they landed in `craftcms` with a cross-reference from craft-pest's site-fixture guidance.

Two corrections to the incoming notes, from reading source: the timezone clobber happens in `ApplicationTrait::_setTimeZone()` (→ Yii's `Application::setTimeZone()`, which *is* `date_default_timezone_set()`), not in `Craft::createObject()`; and `Sites::deleteSite()` doesn't prune category-group config itself — an `ApplicationTrait` listener on `EVENT_AFTER_DELETE_SITE` does, and it's skipped entirely while `getIsApplyingExternalChanges()` is true.

### Added

- **`skills/craft-pest/references/isolation.md`** — **Pin the process timezone after the app is created.** Craft's init resolves `generalConfig->timezone ?? projectConfig->get('system.timeZone')` and hands it to `setTimeZone()`, which is `date_default_timezone_set()`; `craft\migrations\Install` seeds `'timeZone' => 'America/Los_Angeles'`, so a from-scratch test database gives the process a UTC-8/-7 clock. Datetimes built afterward round-trip through saved element attributes shifted by the full offset, and it presents as intermittent expiry logic rather than anything timezone-shaped. Pin UTC *after* app creation so the pin wins; Craft core's own example test-suite bootstrap does the same.
- **`skills/craft-pest/references/isolation.md`** — **Force the database name, default everything else.** Force only what local dev gets wrong (`CRAFT_DB_DATABASE`, `CRAFT_DB_TABLE_PREFIX`); declare driver/server/port/user/password with PHPUnit's `default="true"` so a CI job's own service env flows through. A forced local hostname (`value="db"`) is right inside DDEV and unreachable on a GitHub Actions runner — and if you've reached for an `/etc/hosts` alias to make it resolve, that's the signal to un-force the value rather than patch the runner.
- **`skills/craft-pest/references/craft-state.md`** — **Site fixtures are per-test, never bootstrap-created.** A bootstrap-created site is project-config-backed and survives the rollback, permanently mutating the test database and leaving CI as the only place the fresh-DB path ever runs. Create sites in the test, delete them in a global `afterEach()` — which runs *before* the rollback: `Pest\Concerns\Testable::tearDown()` invokes `afterEach` and only then `parent::tearDown()` (craft-pest's `TestCase::tearDown()` → `tearDownRefreshesDatabase()`) inside a `finally`.
- **`skills/craft-pest/references/craft-state.md`** — **Service caches go stale when craft-pest swaps components.** `RequestHandler::registerWithCraft()` passes a config *array* for `urlManager` to `setComponents()`, so the next `get()` builds a new instance. Class-level `Event::on()` handlers survive; instance-level listeners stay bound to the discarded object and never fire again, while the replacement's memos start from state a fixture already changed. Give any service with process-lifetime memoization an explicit `reset()` (mirroring `UserPermissions::reset()`) and call it from the fixture directly. Craft core acknowledges the same hazard — its project-config listeners go through a `_proxy()` indirection specifically so the right component is called "if it happens to get swapped out (e.g. for a test)".
- **`skills/craft-pest/references/shared-state.md`** — **Plugin `Install` migrations must be idempotent.** A standalone harness re-invokes `Install::safeUp()` on every process boot whenever plugin-install detection misses, and MySQL accepts unlimited duplicate indexes over the same columns as long as the names differ — so keys accumulate silently until the 64-per-table ceiling makes installs fail on a line that always worked. Guard with `Migration::createIndexIfMissing()` and a `Db::findForeignKey() === null` check, and regression-test by running `safeUp()` twice and asserting index/FK counts stay flat (with `$schema->refresh()`, or you re-read the cached pre-migration shape and the test passes regardless).
- **`skills/craftcms/references/architecture.md`** — **Creating or deleting sites at runtime**, covering the two caches that don't refresh themselves. `getIsMultiSite()` keeps separate `_isMultiSite` and `_isMultiSiteWithTrashed` memos; `Sites::refreshSites()` calls `getIsMultiSite(true)` with `$withTrashed` defaulting to **false**, while `ElementQuery::beforePrepare()` gates its `siteId` WHERE clause on `getIsMultiSite(false, true)` — the variant nothing refreshed. So on a single-site install, code that creates a site and dutifully calls `refreshSites()` gets element queries that silently stop filtering by site for the rest of the process. Also: `deleteSite()`'s prune (via the `EVENT_AFTER_DELETE_SITE` listener) writes to project config but never nulls `Entries::$_sections` / `Categories::$_groups`, so in-process section and category-group models still carry the deleted site; and ordering matters, because `Categories::saveGroup()` throws when `siteSettings` omits any currently-existing site — delete the site first.
- **`skills/craftcms/references/migrations.md`** — **Re-runnable index creation and MySQL's 64-key ceiling**, plus a `Db::findForeignKey()` guard alongside the existing Foreign Keys guidance. The core-side companion to the craft-pest item: where the double execution comes from, how to diagnose an already-polluted table, and why `craft up` normally protects you.

### Changed

- **`skills/craft-pest/SKILL.md`** — description rewritten **1,832 → 1,500** characters. It had grown past the ~1,536 Claude Code reads for routing (see 1.9.2), so its tail was inert; the new trigger phrases were added by cutting "Covers:" prose that duplicated the trigger list. Isolation Checklist gained four rows (timezone, forced-vs-defaulted DB env, guarded `Install::safeUp()`, per-test sites), plus new task routes and a fourth entry under "What a Green Suite Does Not Prove".
- **`skills/craftcms/SKILL.md`** — description extended with the runtime-site triggers (1,265 → 1,389, inside the window); new pitfalls for both core facts, task routes for runtime site work and the 64-key failure, and a `migrations.md` route.
- **`skills/craftcms/references/migrations.md`** — a stale `testing.md` cross-reference now points at the `craft-pest` skill's `shared-state.md`, where that content moved in 1.11.0.
- **`skills/craft-pest/references/ci.md`** — the database-service block now explains the division of labour with `phpunit.xml.dist`: the workflow `env:` supplies connection coordinates, the suite supplies the database name.

## 1.11.0 -- 2026-07-28

Adds a dedicated **`craft-pest`** skill and folds source-verified runtime gotchas from a multi-plugin estate smoke-test and fix wave into five existing skills. Every claim naming a class or method was read in the installed package source before being written — verified against `markhuot/craft-pest-core` 3.2.2, `craftcms/cms` 5.10.11, `symfony/mailer` 7.4, and `yiisoft/yii2` — not taken from READMEs or type definitions. Minor bump: one new skill, one new reference file, additions across eleven reference files.

The testing content moved rather than duplicated: `craftcms/references/testing.md` previously carried ~540 lines of Pest guidance that overlapped the new material, so it is now a pointer plus Codeception basics, and `craft-pest` is the single source of truth.

### Added

- **`skills/craft-pest`** (new skill, 5 reference files) — testing Craft plugins with Pest, led by isolation rather than assertions, because the dominant failure mode is a suite writing to a database it shouldn't. Two facts drive the design, neither of them stated in craft-pest's own docs: **rollback is opt-in** (`TestCase` mixes in ~15 traits but not `RefreshesDatabase`, so without it every factory write commits permanently), and **the env override is cwd-bound** (`InstallsCraft::loadPhpunitXmlEnvironmentVariables()` reads only `getcwd().'/phpunit.xml(.dist)'` and never parses `--configuration=`, so running a plugin suite from a shared project root discards its DB pins and runs against the live database). Also covers: pinning `CRAFT_DB_DATABASE` before Craft boots via `$_SERVER`/`$_ENV`/`putenv()`; installing the plugin under test (`InstallsCraft` boots Craft but installs nothing of yours); what `RefreshesDatabase` does *not* roll back (DDL, project config, files); ambient-state assumptions (edition pinning via `setEdition()`, scoping count assertions); Craft internals that bite in tests (permission-tree memoization and `UserPermissions::reset()`, why real `loginByUserId()` fails the UA/IP gate under CLI, `UTC_TIMESTAMP()` in raw fixtures, muting *every* audit surface, queue stubs, project-config writes inside rolled-back transactions); factories, HTTP/queue/DB assertions, multi-site, mocking, console and event tests; shared-install hygiene and `Install.php`-vs-migration drift in the test DB; and CI wiring.
- **`skills/craft-php-guidelines/references/authorization-parity.md`** (new) — one shared authorization gate called by every surface (CP controller, console command, GraphQL resolver, queue jobs acting for a user), with a test per surface. Console is not exempt: a documented cron path with no permission check is an unauthenticated capability. GraphQL schema scope is *not* the plugin's permission matrix — resolvers enforce plugin permissions themselves. Self-referential guards (self-approval, self-review) don't exist unless written; they belong in the gate, orthogonal to role checks, with an explicit bypass permission. And parity means the same *plugin* gates, not the same *core* calls — a console path resolving canonical elements must not copy a web path's `canSave()` check that implicitly relies on draft ownership.
- **`skills/craftcms/references/controllers.md`** — reserved request params (`token` collides with `GeneralConfig::$tokenParam`; `Application::init()` throws `BadRequestHttpException('Invalid token')` before routing, so a plugin minting its own `?token=` links gets a blanket 400 that only an HTTP test catches); `renderTemplate()` sets `TemplateResponseFormatter::FORMAT` (`'template'`), not `Response::FORMAT_HTML`, so response-injection listeners gating on the latter fire only on error pages; `WrongEditionException` carries no HTTP status and is a 500 uncaught in `beforeAction()` — catch and rethrow `NotFoundHttpException`, and audit every controller individually; project-config writes throw `BusyResourceException`/`StaleResourceException` under the mutex and should return a 409 rather than a 500; bounding synchronous exports (stream, hard-cap by row count, or queue — never buffer unbounded results into a document renderer).
- **`skills/craftcms/references/cp.md`** — CP template routing bypasses controllers: in CP template mode the private trigger is a hardcoded `'_'` (not the configurable site-mode setting), so any non-underscore-prefixed template in a plugin's `templates/` is directly routable and skips every `beforeAction()` gate. Underscore-prefix everything that isn't an intentional direct route; gate deliberate ones in the template.
- **`skills/craftcms/references/architecture.md`** — `Db::parseParam()` turns a leading or trailing `*` into a SQL `LIKE` wildcard, so a uniqueness check on a stored pattern silently becomes a prefix match (use a raw `andWhere(['col' => $value])`); serialized/hash-chained writers need bounded retry with jittered backoff on MySQL serialization failures (SQLSTATE 40001 / 1213), re-reading the chain head in a fresh transaction per attempt and throwing a distinct exception when retries exhaust so callers requeue instead of dropping.
- **`skills/craftcms/references/events.md`** — cross-plugin event contracts: an open-vocabulary emitter paired with a closed-enum consumer that drops unknown values is invisible data loss (contract-test both directions; silent drops are never acceptable for audit-class data); dual event surfaces (a sink registry *and* an after-record event) are legitimate but both must be documented in the README's events section.
- **`skills/craftcms/references/permissions.md`** — `saveGroupPermissions()` silently drops nested permissions whose parent wasn't submitted (`_findSelectedPermissions()` only recurses into selected parents) and still returns `true`: re-read post-save state rather than trusting the submitted array, and expand full ancestor chains before saving. Plus the per-process memoization of `getAllPermissions()`, which makes dynamically-registered handles look like orphans until `reset()` is called.
- **`skills/craftcms/references/sessions-and-auth.md`** — vetoing impersonation leaks `impersonatorId`: Craft sets it before `loginByUserId()` and clears it only when login returns `false`, but Yii's `login()` returns `!getIsGuest()`, which is `true` for the already-authenticated actor — so the rollback branch is unreachable on a veto and the next request performs a bogus impersonation. Plugins vetoing sign-in-as must clear it themselves.
- **`skills/craftcms/references/drafts-revisions.md`** — `Revisions::createRevision()`'s change-detection guard compares the last revision's `dateCreated` against `elements.dateUpdated`, which a raw `Db::update()` never bumps, so the revision is silently skipped and the existing ID returned. Pass `force: true`.
- **`skills/craftcms/references/email.md`** — `Symfony\Component\Mailer\SentMessage` freezes a clone at construction: `getMessage()` is that snapshot and never reflects later mutations, while `getOriginalMessage()` is the live object. Custom transports must serialize after any mutating event and never reuse an earlier serialization (e.g. one produced for size probing).
- **`skills/craftcms/references/cp-ui-patterns.md`** — fields injected into another form's DOM must be **id-only, no `name`**, or the surrounding element form serializes them and the values are dropped on save; includes the decision table for when a `name` *is* correct (real custom fields on that element, saved by the host form).
- **`skills/craft-garnish/references/integration.md`** — the JS half of the injected-panel pattern: read values by id, `type="button"` never `submit`, an Enter-key guard so the host form doesn't submit, lightswitch state via the widget rather than `:checked`, and `Craft.sendActionRequest` for the CSRF token.
- **`skills/craft-php-guidelines/references/tooling.md`** — Composer hygiene for plugin repos: never ship `../*` path repositories (they resolve only on the author's disk), unpublished sibling deps get a `vcs` entry, Packagist deps need nothing, `composer.lock` stays gitignored for libraries, and standalone resolvability is proven with an empty `composer config --global repositories` followed by a no-lock `composer update --dry-run`. Remove per-repo CI composer-config workarounds once the `vcs` entry exists.
- **`skills/ddev/SKILL.md`** — running a plugin's own suite inside a host project via `ddev exec --dir <plugin-root> vendor/bin/pest`, cross-referencing why the shared `ddev craft pest -- --configuration=…` form defeats per-plugin DB isolation.

### Changed

- **`skills/craftcms/references/testing.md`** — reduced from ~540 lines to a pointer at the `craft-pest` skill plus Codeception basics (base class, fixtures, `mockCraftMethods`, and the `TestSetup`/Codeception autoload trap). Prevents two files describing the same harness and drifting.
- **`skills/craftcms/references/quality.md`** — CI notes now state `check-cs` never `fix-cs` (auto-fixing in CI rewrites files in the runner, exits `0`, and discards the violations) and that the Pest step is not optional, invoked from the plugin's own root; Testing section points at `craft-pest`.
- **Trigger descriptions** — `craft-garnish` (injected-panel/id-only phrases) and `ddev` (`ddev exec --dir`, plugin suite invocation) extended so the new content fires; `craft-pest`'s description validated at 19/20 on a 20-query trigger eval (recall 10/11, all 9 near-misses correctly off).
- **`skills/craft-php-guidelines/SKILL.md`**, **`skills/craftcms/SKILL.md`** — pitfall entries and reference-file routes for the above; `craftcms` now declares `craft-pest` as a companion skill.
- **Manifests and docs** — `craft-pest` registered in `plugin.json`, `marketplace.json`, `README.md`, and `docs/skills-overview.md` (11 → 12 skills).
...
- **`skills/craftcms/references/architecture.md`** — new "Settings belong in project config — including operational settings" subsection under Project Config: operational settings are configuration (project config), not runtime data (DB); reject the "DB-only for per-environment tuning" anti-pattern (use env vars / `config/{handle}.php` for genuinely per-env values); prod/UAT project config is read-only by design; the real failure mode is a downstream DB edit silently reverting on the next `craft up`, fixed by keeping project config authoritative. Includes the "thresholds should be DB-only to avoid cross-env churn" code-review misconception to correct. Plus a Common Pitfalls bullet.
- **`skills/craft-content-modeling/references/infrastructure.md`** — cross-referencing bullets in "Project Config Essentials → Rules" (operational settings belong in project config; use `$ENV_VAR` for per-env values, not a DB-only column), pointing to `architecture.md`.
- **`skills/craft-plugins/references/formie.md`** — expanded "Multi-site & translations" with translation-override precedence (a project `translations/<locale>/formie.php` overrides Formie's *bundled* strings via `PhpMessageSource` `allowOverrides`, and reaches the client through `getFrontEndJsTranslations()` → `window.FormieTranslations`); new "Field length limits & their validation messages" subsection (native `limit`/`min`/`minType`/`max`/`maxType` on `SingleLineText`/`MultiLineText`; the two message keys — server `You must enter at least {limit} characters.` and client `{attribute} must be no less than {min} characters.`; the `required`+`min` no-double-message interaction via `skipOnEmpty=false`/`skipOnError=true` under `SCENARIO_LIVE`); new "Editing a field on an existing form (migration)" subsection (`getFormByHandle` → `getFieldByHandle` live-layout mutate → `saveElement($form)`, persisted via `beforeSave`→`saveLayout`; no `Forms::saveForm()`). Verified against `verbb/formie` 3.1.30.
- **`skills/craft-site/references/multi-site-patterns.md`** — a note in Static Translations that a project `translations/<lang>/<category>.php` overrides a plugin's *bundled* category strings (Craft registers plugin categories with `allowOverrides`; project wins), including plugin front-end JS strings piped through `Craft::t`.
- **`skills/craftcms/SKILL.md`**, **`skills/craft-plugins/SKILL.md`** — task-routing rows and trigger keywords for both topics (project-config settings storage / YAML↔DB divergence; Formie translation overrides, min/max validation messages, `required` vs `min`, editing a form field in a migration).

## 1.7.2 -- 2026-07-01

Documents a production-bug class verified against `craftcms/cms` 5.x: **element/field type registration must not be gated by request context.** A plugin registered `Elements::EVENT_REGISTER_ELEMENT_TYPES` inside an `if (getIsCpRequest())` branch in `init()`, so its custom element types existed only for Control Panel requests. `Elements::getAllElementTypes()` is per-request, so in a **console** request (e.g. `php craft gc/run --delete-all-trashed`) the type was missing from that list — and `Gc::hardDeleteElements()` deletes trashed rows with `['type' => $normalElementTypes]` built from it, silently skipping any absent type. Result: trashed rows of the custom element accumulated indefinitely (5,170 in the real incident). The same trap affects `resave/*` and index maintenance from queue jobs (the queue runs in a console context).

### Changed

- **`skills/craftcms/references/events.md`** — new "Registration scope — never gate element/field type registration by request context" subsection under Registration Pattern. States the rule (register `EVENT_REGISTER_ELEMENT_TYPES` / `EVENT_REGISTER_FIELD_TYPES` unconditionally in `init()`, with a WRONG example); the consumers that silently misbehave when a type is missing in a context (`Gc::hardDeleteElements()` — the headline; `resave/*`; queue-run element/search-index maintenance); what legitimately *stays* gated behind `getIsCpRequest()` (CP URL rules, asset bundles, nav/sidebar, CP template hooks, CP-only user field layouts) with the "defines what it *is*" vs "renders/routes the CP" dividing line; a `php craft shell` detection assertion (`in_array(MyElement::class, …getAllElementTypes(), true)`); and the note that a plugin's own `Gc::EVENT_RUN` hook can't compensate (trashed rows are excluded from element queries by default). Verified against `Gc::hardDeleteElements()` and `Elements::getAllElementTypes()`. Added a Common Pitfalls bullet and Contents entry.
- **`skills/craftcms/SKILL.md`** — two task-routing rows (incl. the "trashed custom elements never get deleted / gc not purging" symptom) and trigger keywords (`EVENT_REGISTER_ELEMENT_TYPES` scope, element/field type registration context, trashed elements never deleted, `gc/run` not purging custom elements, `getAllElementTypes` empty in console, `hardDeleteElements` skips type).
- **`skills/craft-php-guidelines/SKILL.md`** — a Common Pitfalls bullet framing the rule as an `init()`/plugin-structure concern, cross-linked to the `craftcms` skill's `events.md` → "Registration scope".

## 1.7.1 -- 2026-06-30

Fills two confirmed CP-extension documentation gaps, both verified against source — Craft 5 `craftcms/cms` for the PHP/Twig surface, and the Garnish library (read from its bundled source map) for the JavaScript. **(1)** How to render custom UI into an element *edit screen* — the skills already noted that `EVENT_DEFINE_SIDEBAR_HTML` / `EVENT_DEFINE_ADDITIONAL_BUTTONS` exist but gave no markup, so it was easy to build a panel that renders wrong. The headline trap is the two opposite `.meta` treatments: a titled `fieldset` + `.meta` field card vs. the muted `.meta read-only` readout from `Cp::metadataHtml()`. **(2)** The lifecycle and accessibility of *custom overlays/slideouts* built on `Garnish.Base` — the `disable()` gotcha that silences a component's own trigger listeners, the real `trapFocusWithin`/`releaseFocusWithin`/`setFocusWithin` mechanics, and the `aria-hidden` "descendant retained focus" ordering trap (move focus out before hiding). No new skills or files; additions land in existing reference files.

### Changed

- **`skills/craftcms/references/cp-ui-patterns.md`** — new "Element Edit Screen — Sidebar Panels & Toolbar Buttons" section. Covers the two `.meta` treatments and when to use each (titled panel → `<fieldset><legend class="h6">` + plain `.meta` + `.field`/`.input` rows, rendered as a filled `--gray-050` card in the sidebar; key→value readout → `Cp::metadataHtml()` → `<dl class="meta read-only">` with `.data`/`.value` rows, muted and transparent); appending a titled panel from a plugin via `EVENT_DEFINE_SIDEBAR_HTML` (Craft only auto-wraps the element's own `metaFieldsHtml()`, so appended HTML must carry its own wrapper); a `metaFieldsHtml()` override using `Cp::*FieldHtml()` helpers; and the top-toolbar split button via `EVENT_DEFINE_ADDITIONAL_BUTTONS` (`.btngroup` + `.btn.menubtn[data-disclosure-trigger]` + `.menu.menu--disclosure[data-align="right"]`, the required `.menu-item-label` span, `<ul>`/`<li>` items, and Garnish auto-init via `Craft.initUiElements`). Verified against `Element::getSidebarHtml`/`getAdditionalButtons`, `Entry::metaFieldsHtml`, `Cp::fieldHtml`/`metadataHtml`/`disclosureMenu`, `ElementsController::_additionalButtons`, and the rendered CP CSS/markup.
- **`skills/craftcms/references/elements.md`** — the `EVENT_DEFINE_SIDEBAR_HTML` / `EVENT_DEFINE_ADDITIONAL_BUTTONS` entries in the Element Events Reference now document the `DefineHtmlEvent` append pattern (`$event->html`, `$event->sender`), the not-auto-wrapped caveat, and where the toolbar HTML lands, cross-linked to `cp-ui-patterns.md`.
- **`skills/craftcms/SKILL.md`** — added trigger keywords (element edit sidebar panel, `EVENT_DEFINE_SIDEBAR_HTML` markup, split button toolbar, `.meta` vs `.meta read-only`, `metaFieldsHtml` override, `Cp::metadataHtml`, …), four task-routing rows, and the new coverage to the `cp-ui-patterns.md` table row.
- **`skills/craft-garnish/references/class-system.md`** — expanded "Enable / Disable" with the mechanism (`addListener` gates every handler on `this._disabled`) and the pitfall: one `Garnish.Base` instance owning both a persistent trigger and a transient overlay must not call `disable()` to "pause" the overlay (it silences the trigger too — works once, then dies); shows the broken pattern and two fixes (state-flag guard, or separate instances), and why Garnish's own `Modal` can safely `disable()` (its trigger is external). New Common Pitfalls entry.
- **`skills/craft-garnish/references/utilities.md`** — documented what `trapFocusWithin` actually does (binds only `keydown.focus-trap`, wraps Tab between first/last `:focusable`, does **not** redirect programmatic focus or `focusin`, and is **not** auto-released) and `setFocusWithin`'s `.prevent-autofocus` handling; new "Closing an overlay: move focus OUT before hiding" subsection on the `aria-hidden` "descendant retained focus" browser block and the `inert` alternative. New Common Pitfalls entry.
- **`skills/craft-garnish/references/ui-widgets.md`** — a "Building a custom overlay/slideout on Garnish.Base" lifecycle note (don't disable to pause; focus on open; focus-out before hide on close; `this.base()` on destroy) cross-linking `class-system.md` and `utilities.md`; plus a DisclosureMenu "Accessibility & focus" note (it toggles `aria-expanded` + `display`, not `aria-hidden`, and returns focus to the trigger on close).
- **`skills/craft-garnish/SKILL.md`** — added trigger keywords (custom slideout/overlay lifecycle, disable kills listeners, aria-hidden retained focus, `releaseFocusWithin`, `setFocusWithin`, `inert`, …) and two task-routing rows.

## 1.7.0 -- 2026-06-25

Fixes a plugin-knowledge **discoverability** gap surfaced by a real incident: an agent building a Formie application form in a content migration on Craft Cloud never found the curated `formie.md`, because every plugin reference lived under the front-end `craft-site` skill and no skill description routed a back-end task to it. Two root causes — wrong altitude (back-end tasks don't trigger a front-end skill) and no name-based routing — plus a content gap (`formie.md` was 100% front-end). This release adds a dedicated, altitude-neutral **`craft-plugins`** index skill, relocates the 23 plugin references under it, threads plugin trigger keywords into the back-end skills, and expands `formie.md` with a verified programmatic/back-end section. A side benefit: the references are now flat under a skill's `references/` directory, so they are reachable through the `Skills` PHP API (`references()` / `referenceContent()`) for the first time — previously the nested `plugins/` subdirectory and the slash in `plugins/formie` made them invisible to that API (and thus to `craft-cortex`). No breaking changes: no skill was renamed, the move is internal, and skill discovery scans the filesystem.

### Added

- **`skills/craft-plugins/SKILL.md`** — new index/router skill (the 11th skill). Its `description` enumerates all 23 covered plugins plus altitude-neutral trigger verbs (build, configure, style, render, query, import, migrate, deploy, translate, cache, debug), so a task that names a plugin routes here regardless of whether the work is Twig, PHP, content modeling, or deployment. Body is a routing table mapping each plugin to its `references/<plugin>.md`, with primary-altitude tags.
- **`skills/craft-plugins/references/formie.md`** — new **Programmatic & Back-End (PHP / Migrations / Deployment)** section, verified against the `verbb/formie` `craft-5` source (Formie 3.x / Craft 5): forms are DB elements (not project config) and move cross-environment via JSON Import/Export or a content migration; idempotent form creation (`getFields()->createField()`, `FieldLayout`/`FieldLayoutPage`/`FieldLayoutRow` config arrays, `setFormLayout()`, `setNotifications()`, save via the elements service — there is no `saveForm()`, and `FormLayout` is an alias of `FieldLayout`); the ProseMirror-JSON-not-HTML notification-`content` gotcha; single non-localized element + static translation under the **`formie`** category (not `site`); File Upload volume config and renaming via `Submission::EVENT_AFTER_SAVE` (the asset doesn't exist at `BEFORE_SAVE`) with `Notification::$attachFiles`; captcha/caching (`Formie.refreshForCache()` on `onFormieInit`); and the Cloud cache-flush caution. Three claims that couldn't be confirmed from source (Turnstile cache-safety, notification render-language, `attachAssets` semantics) are explicitly flagged rather than asserted.

### Changed

- **Relocated all 23 plugin references** from `skills/craft-site/references/plugins/*.md` to `skills/craft-plugins/references/*.md` (flat). `craft-site`'s plugin table is replaced with a pointer to the `craft-plugins` skill; `amazon-ses.md`'s cross-link to `third-party-integration.md` now resolves to the `craft-site` skill.
- **`skills/craftcms/SKILL.md`** — added back-end plugin trigger phrasings to the `description` (building a Formie form in a content migration, programmatic Formie form/field/notification creation, Formie File Upload volume + rename via `Submission EVENT_AFTER_SAVE`, SEOmatic SEO field in PHP, Feed Me import job, Blitz purge from a service, Imager-X transforms in a queue job) and a directive to load `craft-plugins` for plugin-specific PHP/migration/deployment work.
- **`skills/craft-content-modeling/SKILL.md`** — added triggers for Formie forms as elements vs project config, cross-environment Formie deployment, and multi-site Formie form translation.
- **`.claude-plugin/plugin.json`** — registered `./skills/craft-plugins`; bumped to 1.7.0; reconciled the description to 11 skills.
- **`.claude-plugin/marketplace.json`**, **`README.md`**, **`docs/skills-overview.md`**, **`docs/getting-started.md`**, **`docs/contributing.md`** — reconciled counts to 11 skills; added the `craft-plugins` skill row/section; pointed the "add a plugin reference" contributor workflow at the new location and routing table.
...
- **`skills/craftcms/references/elements.md`** — new "Field value storage — and setting relation fields the right way": Craft 5 stores field values in `elements_sites.content` (JSON keyed by the field's **layout-element UID**, not the handle/field UID); for relation fields `dbType()` is JSON and the value is read from that content array, while the `relations` table is a *secondary* `relatedTo`/eager-load index. Writing `relations` directly has zero effect on the rendered value — set via `setFieldValue()` + `saveElement()` (a content migration on Cloud), with multi-site `sourceSiteId` semantics. New trigger + pitfall. Verified against 5.10.5 (`BaseRelationField::dbType/serializeValue/normalizeValue/afterElementSave`, `relations` schema).
- **`skills/craftcms/references/migrations.md`** — added a "field values → element API in a content migration" row; scoped the "no element APIs in migrations" pitfall to *schema-phase* migrations (content migrations correctly use the element API).
- **`skills/craftcms/SKILL.md`** — added triggers for the field-storage topic (`set relation via DB`, `relations table`, `field value empty after DB insert`, `elements_sites.content`, `layout element UID`, `multi-instance field value`, …).
- **`skills/craft-content-modeling/references/infrastructure.md`** — pointed the JSON-by-instance-UID discussion at `elements.md`; rewrote the "move field data in migrations" bullet to mandate the element API over hand-edited JSON/`relations`.
- **`skills/craft-cloud/references/caching-and-edge.md`** — new "Two cache layers: data cache vs edge": the Craft data cache (Redis, survives deploys) vs the Cloudflare edge HTML cache; `clear-caches/*` only clears data caches — only `clear-caches/craft-cloud-caches` (`StaticCache::purgeAll()`) reaches the edge; the `?cb=` / `cf-cache-status` origin-MISS diagnostic with the `cache.rules` caveat. Verified against `craftcms/cloud` 3.2.1.
- **`skills/craft-site/references/plugins/seomatic.md`** — new "Rendered Tag-Data Cache (Runtime Mutation Gotcha)": SEOmatic caches each container's rendered tag-data via `getOrSet` (tagged `seomatic_metacontainer`), re-reading a tag's `include`/`content` only on a cache miss — so runtime tag mutation is a no-op against a warm cache (which survives deploys on Redis, ~always-miss locally under devMode/30s). Two-step fix (`clear-caches/seomatic-metabundle-caches` then `clear-caches/craft-cloud-caches`) and the hard-coded `@` in the `twitter:creator` template. Verified against seomatic 5.1.21.
- **`skills/craft-site/references/javascript-boundaries.md`** — new "SSR-Seed Hydration": always paint the SSR seed on the first frame and reconcile in the background (stale-while-revalidate); don't gate rehydration on "no query string"; watch for artificial loading-flag delays; test the clean path in an isolated browser context.
- **`skills/craft-content-modeling/SKILL.md`** + **`skills/craft-site/references/search.md`** — limits of field reuse: the search index can't distinguish field instances ([documented](https://craftcms.com/docs/5.x/system/searching.html#multi-instance-fields); `summary::daisy` may match a `byline` instance), so apply the single-responsibility test instead of reusing always. Noted that element-query filtering by the overridden handle works — only the search index is instance-blind.
- **`skills/craftcms/references/testing.md`** — integration tests must self-seed every row they depend on (incl. FK parents); a green local run isn't authoritative when the dev DB carries seed data — confirm on CI. Documented the `RefreshesDatabase` transaction-rollback and delete-then-insert pattern.

### Fixed

- **`skills/craftcms/references/drafts-revisions.md`** (verified against `craftcms/cms` 5.10.5) — "Applying Drafts" was wrong: the canonical is validated under `SCENARIO_ESSENTIALS`, not LIVE, so applying a draft does **not** enforce required field-layout content (validate under `SCENARIO_LIVE` yourself to gate promotion). Documented the default draft exclusion (`$drafts = false` → `WHERE draftId IS NULL`) and the `drafts(null)->draftOf(false)` pattern for fetching unpublished drafts; clarified `saveElementAsDraft` validation; corrected the Status-Checking table — `getIsCanonical()` is `!isset($_canonicalId)` and is **true for unpublished drafts**, `getIsUnpublishedDraft()` = `getIsDraft() && getIsCanonical()`, and `getIsDerivative()` is false for unpublished drafts.
- **`skills/craftcms/references/caching.md`**, **`skills/craftcms/references/graphql.md`** — fixed two dead TOC anchors (heading-punctuation slug mismatch).
- **`skills/craft-site/references/boilerplate-routing.md`** — removed a dangling reference to a nonexistent `craft-ds-vision.md` (private-project leak).
- **`skills/craft-cloud/references/caching-and-edge.md`** — added the missing TOC sub-entry for the new section.

## 1.6.3 -- 2026-06-18

Documents a Craft Cloud deploy hazard that self-hosted habits hide — flushing the whole cache from a migration — and corrects the internal `craft up` execution order, previously documented as a two-step "migrations then project config" sequence. The real order (verified against `craftcms/cms` `UpController`) runs the **content** migration track *last*, after project config is applied, so content migrations can rely on YAML-defined schema while plugin/Craft migrations cannot. Also makes the Cloud cache wiring explicit: only the cache uses Redis/Valkey, and only when provisioned — otherwise it falls back to a single MySQL `DbCache` table, which is why a global flush during the Migrate phase (where the old release is still serving live traffic) can deadlock. Separately, documents that reconfiguring Cloud-owned app components (`response`/`session`/`cache`/`queue`/`assetManager` + the DB connection) in `config/app.*.php` collides with the `craftcms/cloud` extension and can return HTTP 500 on every request — and can't be caught locally, since the extension only runs on Cloud. Verified against `craftcms/cloud-extension-yii2` `3.x` `AppConfig.php` and `craftcms/cms` `5.x` `UpController` (deploy/migration items), and the installed `craftcms/cloud` `3.2.1` (`Module.php`, `web/ResponseEventHandler.php`, `AppConfig.php`, `Helper.php`) for the app-component item, on 2026-06-18.

### Changed

- **`skills/craft-cloud/references/extension.md`** — new "Cache, queue, and session wiring" section: the `cache` component uses `RedisCache` only when Valkey/Redis is provisioned (`CRAFT_CLOUD_CACHE_SRV`, then the deprecated `CRAFT_CLOUD_REDIS_URL`) and otherwise falls back to `craft\cache\DbCache` (a MySQL table — Lambda's ephemeral FS rules out `FileCache`); `queue` is `SqsQueue` (**not** Redis); `session` is `DbSession`. Corrects the common "everything's on Redis" misconception. The `cloud/up` step-4 now spells out the internal `craft up` order.
- **`skills/craft-cloud/references/deploy-pipeline.md`** — new "Never flush the whole cache from a migration" subsection under the Migrate phase: during Migrate the previous release still serves live traffic against the shared cache, so a global flush on the `DbCache` fallback can deadlock (MySQL 1205, "Lock wait timeout exceeded") and approach the ≈890s CLI cap. Use targeted invalidation; do full clears as a separate post-deploy Console step. Migrate step-4 now documents the internal `craft up` order.
- **`skills/craft-cloud/references/plugin-development.md`** — new "Don't do synchronous external I/O in save hooks" section: a blocking external call in `Element::EVENT_AFTER_SAVE` / `Elements::EVENT_AFTER_SAVE_ELEMENT` stalls every save and, during a resave or `cloud/up`, serializes network latency into the deploy's CLI cap. Queue the call instead, with a draft/revision/propagating guard.
- **`skills/craft-cloud/SKILL.md`** — three cross-cutting pitfalls (cache flush in migrations; synchronous external HTTP in save hooks; reconfiguring Cloud-owned app components in `config/app.*.php` — collides with the extension's `response`/`session`/`cache`/`queue`/`assetManager` + DB wiring, can 500 every request, with the runtime `EVENT_AFTER_PREPARE` attach as the fix and a "not verifiable locally" caveat), plus a "What's Different" row for app/component config. Folds the former standalone `db.php` pitfall into the app-component principle.
- **`skills/craftcms/references/migrations.md`** — rewrote "Execution order in `craft up`" with the verified six-step sequence and its practical consequence (content migrations run after project config; plugin/Craft migrations before); added the `allowAdminChanges`-off rule (on production/Cloud, don't build project-config-managed schema imperatively in migrations — let `project-config/apply` create it from committed YAML with matching UIDs); new cache-flush pitfall.
- **Stale-reference fix** — `craftcms/cloud-extension-yii2`'s default branch is `3.x`, not `main`; updated the "verified against @main" footers in the touched Cloud reference files.
